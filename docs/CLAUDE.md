...
The agent runners now emit `segment` events for multi-message turns (Claude's mid-task follow-ups, Codex's multiple `agent_message` items). Each segment carries its own timestamp and is rendered as a collapsible block in the frontend. `runOpencode` and `runHermes` use `locateBin()` to find their CLI binary (PATH first, then `~/.opencode/bin/`, `~/.local/bin/`); experimental agents are listed only when detected. The BTW (by the way) sidekick (`server/lib/agents.js` `runBtw`) forks the main Claude session at a read-only permission tier for side questions without disturbing the active task.

**Backend layout**: `server.js` (~900 lines) holds config, middleware, shared state, schedulers, and static serving; the API routes live in `server/routes/{meta,push,fs,chat,btw,sessions,quota}.js`, each exporting a `createXxxRouter(ctx)` factory that receives shared state/helpers through `routeContext`. New JSON state files should use `server/lib/json-store.js` (in-memory cache + atomic tmp/rename writes, 0o600) instead of hand-rolled load/save; new push notifications go through `server/lib/notify.js`, which fans out to both Web Push and FCM.

**Client HTTP pipeline**: `lib/core/backend/api_transport.dart` (stores + headers + error mapping) is shared by `BackendClient` and `CardsService` — reuse it for new API callers instead of copying connection logic. `MachineCredentialsStore` / `DeviceIdStore` / `WorkdirStore` keep **static** in-memory caches because multiple store instances exist across the app; any new write path must invalidate the cache. Heavy work stays off the UI isolate: credential decryption (PBKDF2 600k iterations) runs in `compute()` on native, and history responses over 256 KB are decoded in an isolate. The shared `/api/events` SSE stream has a 90s idle timeout (3× the server's 30s heartbeat) so silently dead connections hit the reconnect path. Shared UI utils: `lib/core/util/error_text.dart` (`friendlyErrorText`), `format_bytes.dart`, `time_format.dart`.
...
- Experimental agents (opencode, hermes) are only listed by `GET /api/agents` when their CLI binary is found on the host. `locateBin()` scans PATH then known fallback dirs (`~/.opencode/bin/`, `~/.local/bin/`) with a 60s cache. The `btw:claude` sidekick has its own scope key and never writes to the main session.
