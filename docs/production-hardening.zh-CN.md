...
- 每个用户/设备单独生成凭证。淘汰旧设备时，在 `server/tokens.json` 里吊销对应
  token，验证无误后删除已吊销记录，不要多人共用一个长期 token。
...
- 明确设置上传/下载上限。默认单文件上传 100 MB，下载 300 MB；只有在代理和网络都能承受时，
  才调整 `UPLOAD_MAX_BYTES`、`DOWNLOAD_MAX_BYTES`。
- 不要做过宽的 CORS 暴露。Relay 的设备 token 是真正的 API 门禁，但把 `CORS_ALLOW_ORIGIN`
  设为应用的确切 origin（或收窄代理配置）可以减少误暴露。
- 可用 `RELAY_FS_ROOTS` 把文件浏览/上传/下载 API 限制在明确的目录列表内。即使不设置，
  Relay 也会拒绝通过文件 API 访问自身的 `tokens.json`、`.env`、`credentials/`、`~/.ssh`
  及 CLI 登录凭证文件，泄露的设备 token 无法借此签发新凭证或窃取主机密钥。
...
- 轮换、吊销或删除 token 后，确认旧设备无法访问，新设备仍能通过 `GET /api/health`。
