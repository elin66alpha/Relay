...
- 同一套 Flutter 客户端同时支持移动端和 Web。窄屏 Web 保持手机式抽屉布局，宽屏 Web 使用常驻侧边栏。
...
前置要求：Node.js ≥ 18。Linux 安装还需要 `pm2` (`npm install -g pm2`)；两端使用隧道模式都需要 `cloudflared`。
...
首次打开 app 时，导入后端凭证并输入凭证密码。移动端可以用摄像头扫描二维码。Web 端通过粘贴加密二维码内容或上传保存下来的二维码图片导入，Web 上刻意隐藏摄像头扫码。之后可以在凭证页面继续添加更多机器。
