...
连接之后，Relay 会先进入首页，展示当前机器、最近使用的蜂群、最近使用的智能体会话，以及“开始使用”教程入口。CLI agent 的日常使用集中在一个聊天界面里：你可以切换 Claude Code、Codex、Antigravity、OpenCode 和 Hermes，选择工作目录，保留多个会话，查看 agent 的流式回复（每条消息独立时间戳），在不干扰主任务的前提下提问旁路问题（BTW sidekick），开蜂群（多智能体群聊），管理文件，检查额度。

## 当前能力

- 一个后端可以连接多个前端设备；手机、Web 和桌面端都能回到同一个工作目录和会话。
- 首页展示当前机器、最近蜂群、最近智能体会话，并提供面向非技术用户的“开始使用”入口。
...
- BTW (by the way) sidekick：只读旁路提问，不干扰主任务（支持 Claude、Codex、Antigravity）。
- 蜂群（多智能体群聊）：多个 agent 共享一份记录，用 `@` 召唤发言；同一条消息里召唤多个成员时可并行运行，且每个成员看到同一份对话快照。每个蜂群可单独选择工作目录和各成员的模型/思考深度/权限。
- 文件系统入口可以浏览后端机器上的目录、切换工作路径、上传和下载文件。
- 额度弹窗、定时消息和系统通知帮助你跟踪 Claude Code、Codex、Antigravity 的可用时间。
- 设置页包含主题、语言、全局字体大小、通知偏好、“开始使用”教程和关于信息。
- 设备 token 管理会显示最近使用设备，并支持删除已经吊销的 token。
